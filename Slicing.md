...
Auch Wandelbarkeit setzt klare Schnittstellen und überschaubaren Scope voraus. Beides fördert testbare Module.
...
Ohne einen systematischen Ansatz, der zur Aufdeckung von Unklarheiten führt, können Sie allerdings nie sicher sein, dass sie schon alle aufgedeckt sind. User Stories und Use Cases sind dafür zu weit weg vom Code. Hier setzt nun Slicing an, das ich Ihnen in dieser Artikelserie vorstelle. Es bietet eine viel tiefere Zerlegungshierarchie und ist zudem auf jeder Ebene in softwaretechnischen Mitteln zur Umsetzung gegründet (Slicing Part 5 Bild 3).
...
Kontexte sind nur relevant für sehr umfangreiche Softwaresysteme mit vielen unterschiedlichen Benutzergruppen. Sie sind leicht zu übersehen, weil das Kennzeichen für sie subtil sein kann. Sehen Sie sich als Beispiel die Amazon-Homepage in Slicing Part 5 Bild 4 an. Sie steht für ein wirklich großes Softwaresystem; so groß, dass Sie es wohl gar nicht als ein Softwaresystem, sondern als Ökosystem von Softwaresystemen bezeichnen würden. Genau das ist aber mein Punkt: Oft fühlen sich große Software­ systeme aus dem Stand nicht monolithisch an. Nur: Woraus bestehen sie dann, was sind ihre Teile? Ist nach dem Überblick über das ganze System sofort ein Sprung auf die App-Ebene angezeigt? Ist die Homepage von Amazon eine App und die Prime-Video-App für den Desktop eine andere? Ich denke, der Sprung wäre viel zu groß. Nicht dass sich keine App erkennen ließe – doch gibt es vielleicht noch weitere? Wenn Apps sich auf eine Rolle konzentrieren, um deren Benutzern ein bestmögliches Bedienerlebnis zu bieten, lässt sich dann für das „Amazon-Softwareökosystem“ die Zahl der Apps einfach nach einem Überblick bestimmen (Slicing Part 5 Bild 5)? Nein, das ist unmöglich. Mit dem Bild bekommen Sie bestimmt schon ein Gefühl dafür, wie verrückt es wäre, Apps erkennen zu wollen. Es braucht vorher mindestens eine gröbere Zerlegung des Gesamtsystems. Das Bild kann nicht alle Benutzerrollen und auch nicht alle Ressourcen listen. Ein so großes System wird zum Glück nie an Sie zur Entwicklung herangetragen. Es kann nicht mit einem Masterplan geschaffen werden, sondern ist Ergebnis einer jahrzehntelangen Bottom-up-Entwicklung. Was heute als Ganzes (Systemlandschaft) vorliegt, ist zusammengewachsen. Es bestand schon aus Teilen, die später „nach oben hin“ zu einem Ganzen verbunden wurden. Lassen Sie uns trotzdem Amazon als Beispiel nutzen, weil es so greifbar und vertraut ist. Was also tun, um Anforderungen an ein (unbekannt) großes System (halbwegs) in den Griff zu bekommen? Im Slicing steht am Anfang die Zerlegung in Kontexte. Sie entsprechen den Bounded Contexts des Domain Driven Design (DDD) [1] [2] (Slicing Part 5 Bild 6).
TODO Seite 5
